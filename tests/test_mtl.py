...
class TestMTL(unittest.TestCase):
    def test_fit_predict(self):
        ...
        model = MultiTask(
            tasks={"sst": Classifier, "qqp": Comparison},
            n_epochs=2,
            optimizer="AdamaxW",
            max_length=200,
        )
        ...
        q_X1, q_X2, q_Y = (
            q_dataset.Text1.values,
            q_dataset.Text2.values,
            q_dataset.Target.values,
        )
        ...
        model.fit(
            {"sst": trainX, "qqp": list(zip(q_X1, q_X2))}, {"sst": trainY, "qqp": q_Y}
        )

        model.featurize({"sst": testX, "qqp": list(zip(q_X1, q_X2))[:10]})

        preds = model.predict({"sst": testX, "qqp": list(zip(q_X1, q_X2))[:10]})
