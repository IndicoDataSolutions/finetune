...
class TestHuggingFace(unittest.TestCase):
    ...
    def check_embeddings_equal(
        self, finetune_base_model, hf_model_path, decimal=2, use_fast=True, **kwargs
    ):
        ...
        hf_seq_features = self.huggingface_embedding(
            self.text, hf_model_path, use_fast=use_fast, **kwargs
        )[0]
    ...
    def test_t5_s2s_ner(self):
        ...
        texts, annotations = finetune_to_indico_sequence(
            raw_docs, texts, labels, none_value="<PAD>"
        )
        ...
        pred_correct = False
        for t, p, l in zip(
            test_texts, finetune_model.predict(test_texts), test_annotations
        ):
            seps_included = seps_included or " | " in p  # check it predicts separators
            pred_correct = pred_correct or any(
                li in p for li in l.split(" | ")
            )  # at least one extraction is predicted correctly.
    ...
    @pytest.mark.skip(
        reason="Longformer is broken, see comments in huggingface/models.py"
    )
    ...
    def test_layoutlm(self):
        ...
        def subset_doc(doc):
            ...
            for page in doc:
                ...
                page["pages"][0]["doc_offset"]["end"] = (
                    page["pages"][0]["doc_offset"]["start"] + page_idx_end
                )
                return [page]
