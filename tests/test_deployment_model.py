...
class TestDeploymentModel(unittest.TestCase):
    ...
    classifier_dataset_path = os.path.join("Data", "Classify", "SST-binary.csv")
    sequence_dataset_path = os.path.join("Data", "Sequence", "reuters.xml")
    ...
    def setUpClass(cls):
        ...
        # dataset preparation
        cls.classifier_dataset = pd.read_csv(
            cls.classifier_dataset_path, nrows=cls.n_sample * 10
        )
        ...
        try:
            cls.s = SequenceLabeler.load(
                cls.sequence_labeler_path, **cls.default_seq_config(cls)
            )
        ...
        if cls.do_comparison:
            ...
            for i in range(n_per):
                different.append(
                    [random.choice(cls.animals), random.choice(cls.numbers)]
                )

            targets = np.asarray([1] * len(similar) + [0] * len(different))
            data = similar + different

            cls.x_tr, cls.x_te, cls.t_tr, cls.t_te = train_test_split(
                data, targets, test_size=0.3, random_state=42
            )

            try:
                cls.cr = ComparisonRegressor.load(
                    cls.comparison_regressor_path, **cls.default_config(cls)
                )
            except FileNotFoundError:
    ...
    def test_reasonable_predictions(self):
        ...
        if self.do_comparison:
            ...
            model = DeploymentModel(
                featurizer=self.base_model, **self.default_seq_config()
            )
            model.load_featurizer()
            model.load_custom_model(self.comparison_regressor_path)
            deployment_preds = model.predict(self.x_te)
            model.close()
            compregressor = ComparisonRegressor.load(
                self.comparison_regressor_path, **self.default_comp_config()
            )
