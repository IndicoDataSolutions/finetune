...
def test_train_error_on_different_lengths(
    get_untrained_classifier, classification_text_sample
):
    model = get_untrained_classifier()
    train_sample = classification_text_sample()
    ...
    with pytest.raises(FinetuneError):
        model.fit(train_sample.Text, train_sample.Label[:1])

    model.fit(train_sample.Text.values, train_sample.Label.values)


def test_class_weights(
    get_untrained_classifier, trained_classifier, classification_text_sample
):
    # testing class weights
    train_sample = classification_text_sample()
    valid_sample = classification_text_sample()
    predictions = trained_classifier.predict(valid_sample.Text.values)
    recall = recall_score(valid_sample.Label.values, predictions, pos_label="Positive")
    model = get_untrained_classifier(class_weights={"Positive": 100})
    model.fit(train_sample.Text.values, train_sample.Label.values)
    predictions = model.predict(valid_sample.Text.values)
    new_recall = recall_score(
        valid_sample.Label.values, predictions, pos_label="Positive"
    )
...
def test_chunk_long_sequences(mock_get_keras_model, get_untrained_classifier):
    ...
    labels = ["a", "b"]
    model = get_untrained_classifier(
        chunk_long_sequences=True, max_length=10, batch_size=1, predict_batch_size=1
    )
    model.finetune(test_sequence, labels)
    assert len(mock_get_keras_model) == 1
    assert len(mock_get_keras_model[0].fit_calls) == 1
    data_lengths = [
        data[0]["tokens"].shape[1]
        for data in mock_get_keras_model[0].fit_calls[0]["data"]
    ]
    assert all(
        length == 10 for length in data_lengths
    )  # For fit we pad everything to max_length for XLA.

    seq_1_num_tokens = len(
        model.input_pipeline.text_encoder.encode_multi_input(
            [test_sequence[0]], max_length=sys.maxsize
        ).tokens
    )
    seq_2_num_tokens = len(
        model.input_pipeline.text_encoder.encode_multi_input(
            [test_sequence[1]], max_length=sys.maxsize
        ).tokens
    )
    # Greater than for overlap and padding.
    assert (
        sum(data_lengths)
        >= (seq_1_num_tokens + seq_2_num_tokens) * model.config.n_epochs
    )

    class_counts = [
        tf.argmax(data[1][0], axis=-1).numpy().tolist()
        for data in mock_get_keras_model[0].fit_calls[0]["data"]
    ]
    ...
    assert (
        len(mock_get_keras_model[0].predict_calls)
        >= (seq_1_num_tokens + seq_2_num_tokens) // 10
    )
...
def test_featurize(trained_classifier, classification_text_sample):
    ...
    assert all(
        feature.shape == (trained_classifier.config.n_embed,) for feature in features
    )
    ...
    assert all(
        feature.shape[1] == trained_classifier.config.n_embed
