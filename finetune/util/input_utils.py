...
def validation_settings(dataset_size, batch_size, val_size, val_interval, keep_best_model):
    """
...
def wrap_tqdm(gen, mode, n_epochs, val_size, dataset_size, skip_val=False, silent=False, update_hook=None):
    ...
    def internal_gen():
        ...
        it = iter(gen())
        if mode == "train":
            desc = "Epoch {}/{}".format(current_epoch, n_epochs)
        else:
            desc = "Validation"
        if skip_val:
