...
def sentences_heatmap(sentences, probs, clf):
    fig, axes = plt.subplots(
        len(sentences), 1, figsize=(20, len(sentences) * 0.95), dpi=160, squeeze=False
    )
    axes = axes[:, 0]
    ...
    for i, ax in enumerate(axes):
        ...
        for j, (word, prob) in enumerate(zip(sentences[i], probs[i])):
            ...
            word_pos += 0.0036 * (len(word) + 1)  # to move the word for the next iter
        ax.axis("off")
...
if __name__ == "__main__":
    ...
    for sample, text in zip(samples, testX):
        ...
        sample_probs = []
        for s, e, p in zip(
            sample["token_starts"], sample["token_ends"], sample["explanation"][1]
        ):
