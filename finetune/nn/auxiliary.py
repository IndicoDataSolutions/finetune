...
def add_timing_signal_from_position(x, position, timescales):
    ...
    for dim, timescale in zip(range(num_dims), timescales):
        ...
        inv_timescales = min_timescale * tf.exp(
            tf.cast(tf.range(num_timescales), dtype=tf.float32)
            * log_timescale_increment
        )
        position_x = tf.expand_dims(
            tf.cast(position[:, :, dim], dtype=tf.float32), 2
        )  # batch, len, 1 # where 1 will be the chanels dim
        scaled_time = position_x * tf.expand_dims(
            tf.expand_dims(inv_timescales, 0), 0
        )  # batch , len, num_timescales
        signal = tf.concat(
            [tf.sin(scaled_time), tf.cos(scaled_time)], axis=2
        )  # batch channels//num_dims
...
def embed_position(context, context_channels, batch, seq):
    ...
    if context_channels is None:
        raise ValueError(
            "context_channels is not set but you are trying to embed context"
        )
    ...
    def get_pos_embed():
        return add_timing_signal_from_position(
            x,
            context,
            timescales=[[(math.pi / 2) * (1 / 2500), (25 * math.pi) * (1 / 2500)]]
            * context_dim,
    ...
    pos_embed = tf.cond(tf.equal(seq, 0), true_fn=identity, false_fn=get_pos_embed)
    return pos_embed
