    ...
    def get_train_input_fns(
        ...
        for task_name in self.config.tasks:
            ...
            val_funcs[task_name + "_train"] = get_train_eval_dataset(
                input_func_normalised, val_sizes[task_name]
            )

        sum_frequencies = sum(frequencies)
        weights = [float(w) / sum_frequencies for w in frequencies]
        train_dataset = lambda: tf.data.experimental.sample_from_datasets(
            [f() for f in input_funcs], weights
        )
...
class MultiTask(BaseModel):
    ...
    def __init__(self, tasks, **kwargs):
        ...
        self.config.task_name_to_id = dict(
            zip(self.config.tasks.keys(), range(len(self.config.tasks)))
        )
    ...
    def _target_model(
        ...
        **kwargs
        ...
        for task in config.tasks:
            pred, loss_logits = get_loss_logits_fn(
                task, featurizer_state, config, targets_i, train, reuse, task_id
            )
        ...
        return {"logits": losses_logits[1], "losses": losses_logits[0]}
