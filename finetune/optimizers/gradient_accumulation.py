...
def get_grad_accumulation_optimizer(
    optimizer_class, accum_steps, accumulate_on_cpu=False
):
    """
    ...
    class GradAccumulationOptimizer(optimizer_class):
        ...
        def apply_gradients(
            self, grads_and_vars, global_step=None, name=None, *args, **kwargs
        ):
            ...
            for g, v in grads_and_vars:
                ...
                with tf.device("cpu") if accumulate_on_cpu else nullcontext():
                    ...
                    try:
                        ...
                        raise FinetuneError(
                            "GradAccumulationOptimizer does not currently support multiple GPUs"
                        )
            ...
            global_step = (
                global_step
                if global_step_set
                else tf.compat.v1.train.get_or_create_global_step()
            )
            if global_step_set:
                ...
                def apply_grads():
                    with tf.control_dependencies(
                        ...
                        apply_grads_op = tf.group(
                            *[g.assign(g.initial_value) for g in accumulation_vars]
                        )
